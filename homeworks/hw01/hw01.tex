...
\title{MATH 341/641 Fall \the\year{} Homework \#1}

\author{Professor Adam Kapelner} %STUDENTS: write your name here

\iftoggle{professormode}{
\date{Due by email 11:59PM Sunday, September 9, \the\year{} \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
\easysubproblem{In the iPhone yes/no survey we took in class, what were the possible values of a datum for each individual?}\spc{0.5}


\easysubproblem{For the iPhone yes/no survey we did in class, provide two possible \qu{representative populations}.}\spc{3}
...
\intermediatesubproblem{What are some problems with SRS's in the real world? Use Google.}\spc{3}

\intermediatesubproblem{Explain why assuming the population size is infinite gives you an $\iid$ DGP when you use an SRS to sample individuals.}\spc{3}
...
\easysubproblem{What is the reason why someone would want to perform point estimation?}\spc{1}
...
\intermediatesubproblem{For any DGP $\Xoneton$ which is independent and identically distributed where $\theta = \expe{X}$, we introduced the estimator $\thetahat = \Xbar$ that can be used to infer $\theta$. Think of another estimator that can be used to infer $\theta$.}\spc{0.5}

\easysubproblem{For any DGP $\Xoneton$ which is identically distributed where $\theta = \expe{X}$, prove that $\thetahat = \Xbar$ is always \emph{unbiased}.}\spc{4}

\easysubproblem{If the DGP is $\Xoneton \iid \normnot{\theta}{\sigsq}$, the normal distribution, show that the the risk under squared error loss for $\thetahat = \Xbar$ is the same regardless of $\theta$. Marked easy because it uses the previous question.}\spc{3}


\hardsubproblem{[MA] Consider the scenario in 1(j). Sample subpopulation A with size $n_A$, sample subpopulation B with size $n_B$, sample subpopulation C with size $n_C$ and sample subpopulation D with size $n_D$, four different quantities such that $n = n_A + n_B + n_C + n_D$. Create an unbiased estimator for $\theta$ using these four samples (and prove that it is unbiased).}\spc{6}

\intermediatesubproblem{If the DGP is $\Xoneton \iid \bernoulli{\theta}$ and we choose the estimator $\thetahat_{\text{bad}} = 1/2$ (that is, degenerately 1/2 with probability 1). Graph the bias of $\thetahat_{\text{bad}}$ over all $\theta$. Label your axes.} \spc{6}

\intermediatesubproblem{Graph the MSE of $\thetahat_{\text{bad}}$ over all $\theta$. And graph the MSE of $\Xbar$ over all $\theta$ was $n=10$. Label your axes. Do you see why $\thetahat_{\text{bad}}$ is a bad choice relative to $\Xbar$?}\spc{6}


\intermediatesubproblem{Compare $\thetahat_{\text{bad}}$ to $\Xbar$ when $n=10$ using the sup risk with squared error loss. How much better is $\Xbar$?}\spc{3}\\

\hardsubproblem{[MA] Invent a loss function that would make the risk for $\thetahat_{\text{bad}}$ better than $\Xbar$ for all $\theta$.}\spc{10}
...
\easysubproblem{What are the three steps we mentioned to generate a test? Write them in the order we mentioned in class.}\spc{3}

\easysubproblem{For the third step, how do you know how far is \qu{a far enough} departure? You need an additional parameter $\alpha$ that you (the statistician) is responsible for supplying to the theory testing.}\spc{3}

\easysubproblem{What is the role of $\alpha$ in a test?}\spc{2}

\easysubproblem{Although $\alpha$ is fundamentally up to you, what is the scientific community's standard(s)?}\spc{1}
...
\easysubproblem{What are the two possible outcomes of a hypothesis test? And which set does $\thetahathat$ belong to in each of these two possible outcomes?}\spc{2}

\easysubproblem{For each of these two possible results, which error could be made? Give the names of the errors and describe them conceptually.}\spc{5}
...
\begin{itemize}
\item $\thetahat = \theta$
\item $\thetahathat = \theta$
\item $\thetahat \neq \theta$, $H_0$ is rejected
\item $\thetahathat \neq \theta$, $H_0$ is rejected
\item If $\thetahat \in RET$, $H_0$ is rejected
\item If $\thetahat \in RET$, $H_0$ fails to be rejected
\item If $\thetahathat \in RET$, $H_0$ is rejected
\item If $\thetahathat \in RET$, $H_0$ fails to be rejected
\item $\prob{\thetahat \in RET} = \alpha$
\item $\prob{\thetahat \notin RET} = \alpha$ 
\item $\prob{\thetahathat \in RET} = \alpha$
\item $\prob{\thetahathat \notin RET} = \alpha$ 
\item $\prob{\text{Type I error}} = \alpha$
\item $\prob{\text{Type II error}} = \alpha$ 
\item $\prob{\text{Type I error}} + \prob{\text{Type II error}} = \alpha$ 
\item $\prob{\text{Type I error}} + \prob{\text{Type II error}} = 1$ 
