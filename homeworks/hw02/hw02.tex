...
\date{Due by email 11:59PM September 20, \the\year{} \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
\problem{Here we will do a binomial exact test using the survey data from our class. We want to demonstrate that the iphone users in our class is \textit{greater} than the national average (which is 52.4\%). Recall that our data was as follows: for $n=21$, the $\thetahathat = 0.476$ where the estimator we chose was the sample proportion.}
...
\easysubproblem{Regardless of if you understood the previous question or not, what is the exact null sampling distribution (write the PMF)? Marked easy because you can copy from MATH 340 class notes.}\spc{0.8}

\easysubproblem{Draw the PMF of the null sampling distribution. Label all axes carefully and provide sufficient tick marks. Marked easy because you can copy from class. Probabilities are below rounded to the nearest 3 digits. Leave space below its horizontal axis.}
...
\hline
0 & 0 & 0 & 0 & 0.001 & 0.006 & 0.016 & 0.039 & 0.074 & 0.118 & 0.156 & 0.172 & 0.158 & 0.120 \\
14 & 15 & 16 & 17 & 18 & 19 & 20 & 21 \\ 
  \hline
0.076 & 0.039 & 0.016 & 0.005 & 0.001 & 0 & 0 & 0
\end{tabular}
\end{table}
...
\easysubproblem{What is the definition of the pval for this 2-sided test?}\spc{2}
...
\easysubproblem{Easy only if you have the answer to the previous question: calculate the power in this test.}\spc{1}
...
\hardsubproblem{Compare $\thetahat_{\text{bad}}$ to $\thetahat$ using the sup risk under squared error loss. How much better is $\Xbar$?}\spc{3}
\end{enumerate}

\problem{We will now do a \emph{one-sided one-sample exact Z test} using the male student height data: $n=13$ and $\xbar = 68.85''$. We want to test at $\alpha = 1\%$ if the population that this sample was drawn from has a \emph{greater} mean than the American female height mean of 65''. According to \href{https://www.usablestats.com/lessons/normal}{this article}, female American height is $\iid \normnot{65}{3.5^2}$.}
...
\hardsubproblem{Why do we use that value of $\theta_0$ when there are potentially infinite values to choose from?}\spc{2}
...
\easysubproblem{Calculate the test statistic on the standardized scale.}\spc{1}

\easysubproblem{Run the test using both RET regions (the original scale and standardized scale). Show both answers are the same.}\spc{1}

\easysubproblem{Calculate the p-val for this test. Use a z-calculator on your graphing calculate or find one on the Internet.}\spc{2}
...
\easysubproblem{If $n \rightarrow \infty$, what does power converge to?}\spc{1}
\end{enumerate}



\problem{Assume male American height is $\iid \normnot{70}{4^2}$ and female American height is $\iid \normnot{65}{3.5^2}$. Using the male and female height data from class, we will now prove that the QC STEM 300-level population mean male height is greater than the QC STEM 300-level population mean female height.}
