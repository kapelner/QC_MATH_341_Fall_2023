...
\date{Due by email 11:59PM Nov 16, \the\year{} \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
\easysubproblem{Why should AICc be employed in this case instead of AIC?}\spc{6}
...
\easysubproblem{Provide the asymptotically normal estimator that is the basis of the score test for one parameter for any iid DGP $f(x;\theta)$.}\spc{3}


\intermediatesubproblem{Show that the score test is equivalent to the Wald test in the case where the DGP is iid $\bernoulli{\theta}$. This means the estimator is the same. Find in the lectures where we derived $I(\theta)$ for the iid $\bernoulli{\theta}$ DGP. Then it's algebraic simplication from there.}\spc{8}
...
\easysubproblem{Show that the LR test is \emph{not} equivalent to the Wald test / Score test in the case where the DGP is iid $\bernoulli{\theta}$. This means the estimator is \emph{not} the same.}\spc{4}


\easysubproblem{Run the LR test for the iid  $\bernoulli{\theta}$ DGP where $n=100$ and $\sum x_i = 61$. Ensure it's different but similar to the result of the score test.}\spc{4}


\easysubproblem{Plot $\thetahathat$ vs $\ell$ and illustrate how the Wald, Score and LR tests can be visualized.}\spc{3}
