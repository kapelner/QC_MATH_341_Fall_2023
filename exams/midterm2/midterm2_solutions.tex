...
\inred{
We can estimate $\doublehat{D}_{n_1, n_2}$ from the $\hat{F}_1(x)$ and $\hat{F}_2(x)$ plots above. At around $x = 60$ it appears as if the difference is slightly less than 3 y-axis gridlines where each has width 0.1. So let's estimate $\doublehat{D}_{n_1, n_2} = 0.28$. We then calculate the test statistic:

\beqn
\sqrt{\frac{n_1 n_2}{n_1 + n_2}} \doublehat{D}_{n_1, n_2} = \sqrt{\frac{51 \cdot 21}{51 + 21}} 0.28 = 1.080 \in \text{RET} = \bracks{0, 1.359}~\Rightarrow~\text{Retain}~H_0
\eeqn
